...
function add($data)
{
    ...
    if (!empty(trim($judul) && !empty(trim($isi)))) {
        $query = "INSERT INTO kuliner VALUES('','$judul','$gambar','$isi')";
...
function edit($data)
{
    ...
    if (!empty(trim($judul) && !empty(trim($isi)))) {
        $query = "UPDATE kuliner SET judul= '$judul', isi = '$isi', gambar = '$gambar' WHERE id = $id ";
        mysqli_query($link, $query);
    }
    return mysqli_affected_rows($link);
}
...
{
    global $link;
    mysqli_query($link, "DELETE FROM kuliner WHERE id = $id");
    return mysqli_affected_rows($link);
}
