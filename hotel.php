...
$jumlahData =  count(query("SELECT * FROM hotel "));
$jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
$halamanAktif = (isset($_GET['halaman']) ? $_GET['halaman'] : 1);
$awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;


$hotel = query("SELECT * FROM hotel LIMIT $awalData,$jumlahDataPerhalaman");

if (isset($_POST['cari'])) {
	$hotel = cariHotel($_POST['keyword']);
}
?>

<header id="gtco-header" class="gtco-cover gtco-cover-sm" role="banner" style="background-image: url(view/images/hotel.jpg)">
			...
			<?php foreach ($hotel as $row) : ?>
				<div class="col-lg-4 col-md-4 col-sm-6">
					<div class="card" style="width: 18rem;">
						<img class="card-img-top" src="admin/hotel/img/<?= $row["gambar"]; ?>" alt="Card image cap" width="286px" height="178px">
						<br><br>
						<div class="card-body">
							<h5 class="card-title"><b><?= $row["judul"]; ?></b></h5>
							<p class="card-text"><?= excerpt($row["info"]); ?></p>
							<a href="detailhotel.php?id=<?= $row['id']; ?>" class="btn btn-primary">Info Detail</a>
						</div>
					</div>
				</div>
				<? $i++; ?>
